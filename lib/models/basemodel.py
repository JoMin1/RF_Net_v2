...
class BaseModel():
    ...
    def __init__(self, opt, data):
        ...
        self.tb_dir = os.path.join(os.path.abspath(os.curdir), 'output', 'skipganomaly', self.opt.dataset, 'tensorboard')
        self.tb_loss_dir = self.tb_dir + '/loss'
        self.tb_auc_dir = self.tb_dir + '/auc'

        if not os.path.exists(self.tb_loss_dir):
            os.makedirs(self.tb_loss_dir)
        if not os.path.exists(self.tb_auc_dir):
            os.makedirs(self.tb_auc_dir)

        self.tb_loss_board = SummaryWriter(self.tb_loss_dir)
        self.tb_auc_board = SummaryWriter(self.tb_auc_dir)
    ...
    def train(self):
        ...
        for self.epoch in range(self.opt.iter, self.opt.niter):
            ...
            res = self.test()
            if res['   recon_AUC'] >= recon_best_auc:
                recon_best_auc = res['   recon_AUC']
            if res['   feat_AUC'] >= feat_best_auc:
                feat_best_auc = res['   feat_AUC']
            if res['   norm AUC'] >= adv_best_auc:
                adv_best_auc = res['   norm AUC']
            if res['   AUC'] >= best_auc:
                best_auc = res['   AUC']
                self.save_weights(self.epoch)
            self.visualizer.print_current_performance(res, recon_best_auc, feat_best_auc, adv_best_auc, best_auc)
