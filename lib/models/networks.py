...
class BasicDiscriminator(nn.Module):
    ...
    def __init__(self, opt):
        ...
        while csize > 4:
            ...
            #                 nn.Conv2d(out_feat, out_feat, 3, 1, 1, bias=False))
            # feat.add_module('extra-layers-{0}-{1}-batchnorm'.format(out_feat, out_feat),
            #                 nn.BatchNorm2d(out_feat))
            # feat.add_module('extra-layers-{0}-{1}-relu'.format(out_feat, out_feat),
            #                 nn.LeakyReLU(0.2, inplace=inplace))
            # """"""
            cndf = cndf * 2
            csize = csize / 2
...
class UnetSkipConnectionBlock(nn.Module):
    def __init__(self, outer_nc, inner_nc, input_nc=None,
        ...
        downconv = nn.Conv2d(input_nc, inner_nc, kernel_size=4,
                             stride=2, padding=1, bias=use_bias)
        downrelu = nn.LeakyReLU(0.2, inplace)
        downnorm = norm_layer(inner_nc)
        uprelu = nn.ReLU(inplace)
...
class UnetNoSkipGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, num_downs, ngf=64,
        ...
        # construct unet structure
        unet_block = UnetConnectionBlock(ngf * 8, ngf * 8, input_nc=None, submodule=None, norm_layer=norm_layer, innermost=True)
        for i in range(num_downs - 5):
            unet_block = UnetConnectionBlock(ngf * 8, ngf * 8, input_nc=None, submodule=unet_block, norm_layer=norm_layer, use_dropout=use_dropout)
        unet_block = UnetConnectionBlock(ngf * 4, ngf * 8, input_nc=None, submodule=unet_block, norm_layer=norm_layer)
        unet_block = UnetConnectionBlock(ngf * 2, ngf * 4, input_nc=None, submodule=unet_block, norm_layer=norm_layer)
        unet_block = UnetConnectionBlock(ngf, ngf * 2, input_nc=None, submodule=unet_block, norm_layer=norm_layer)
        unet_block = UnetConnectionBlock(output_nc, ngf, input_nc=input_nc, submodule=unet_block, outermost=True, norm_layer=norm_layer)
...
class UnetConnectionBlock(nn.Module):
    def __init__(self, outer_nc, inner_nc, input_nc=None,
                 submodule=None, outermost=False, innermost=False, norm_layer=nn.BatchNorm2d, use_dropout=False):
        super(UnetConnectionBlock, self).__init__()
        self.outermost = outermost
        if type(norm_layer) == functools.partial:
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
