...
def load_data(opt):
    ...
    sampler = None
    if opt.num_sample > 0:
        train_ds = random.choices(train_ds, k=opt.num_sample)
        train_dl = DataLoader(dataset=train_ds, batch_size=opt.batchsize, shuffle=True, drop_last=True)
    elif opt.random_sample > 0:
        random_sampler = torch.utils.data.RandomSampler(train_ds,  replacement=True, num_samples=opt.random_sample)
        train_dl = DataLoader(dataset=train_ds, batch_size=opt.batchsize, shuffle=False, drop_last=True, sampler=random_sampler)
    else:
        train_dl = DataLoader(dataset=train_ds, batch_size=opt.batchsize, shuffle=True, drop_last=True)
